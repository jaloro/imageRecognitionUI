...
iniResult = require( "../conf/transfer_ini.json" );
...
fs.mkdir( path.resolve( "./", imgsDir ), { recursive: true }, function( err ){		// 把 __dirname 修改为 "./"
	if ( err ) throw err;
	console.log( '\x1b[32m%s\x1b[0m', "Server Route:", "'" + serverRoute + "'" );
	console.log( '----------------------------------------------------------' );
	...
	console.log( '\x1b[32m%s\x1b[0m', "Rroxy Route:", "'" + proxyRoute + "'" );
...
module.exports = async function ( fastify, options, next ) {
	...
	fastify.post( serverRoute, async ( req, reply ) => {
		...
		formProxy.append( 'file', readStream );			// 添加一个文件型参数到 formData 对象中		// formProxy.append( 'firstName', 'Marcin' );		// 添加其他值对参数到 formData 对象中
