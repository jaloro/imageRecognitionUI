...
// async function appendSync( a_formdata, a_file ) {
// 	return new Promise( ( resolve, reject ) => {
// 		let _readStream = fs.createReadStream( a_file );					// 建立转换写入流， *注：写入流没有监测到 end 事件，需要监测 close 事件
// 		_readStream.on( 'close', () => {
// 			// console.log( "\tConversion of image to 'JPG' format succeeded.".bGreen );
// 			console.log( "\tAppend end.".red );
// 			resolve( { success: true } );
// 		});
// 		a_formdata.append( 'file', _readStream );
// 		// return new Promise( ( resolve, reject ) => {
...
module.exports = async function ( fastify, options, next ) {
	...
	fastify.post( serverRoute, async ( req, reply ) => {
		...
		let receiveParts = new Promise( ( resolve, reject ) => {
			try {
		...
		let saveParts = receiveParts.then( _getPartsSuccess => {
		...
		let convertFormat = saveParts.then( _saveAllArgsComplete => {
				...
				if ( _files.length > 0 ) {
					for ( let _i = 0; _i < _files.length; _i ++ ) {						// 转换后台无法处理的图片格式到 jpg 格式
						...
						if ( _files[ _i ][ "realMime" ] == "image/webp" ) {
							...
							// console.log( "\tDon't Convert image format ...".underline + _i );
						} else {
							continue;			// 非图片格式文件，跳过
						}
						// console.log( _i );
		...
		let appendFiles = convertFormat.then( async _convertFormatSuccess => {
			...
			if ( _files.length > 0 ) {
				...
				for ( let _j = 0; _j < _files.length; _j++ ) {
					if ( _files[ _j ][ "realMime" ].substr( 0, 6 ) == "image/" ) {
						try {
							// let _appendStream;
							// _appendStream = fs.createReadStream( _files[ _j ][ "realPathName" ] );
			...
			return new Promise( ( resolve, reject ) => {
				console.log( "\t'For' ending".red );
				resolve();
			});
		});
		
		appendFiles.then( _appendSuccess => {
			// 提交 form 对象到后台 ===================================================================================
			...
			const _transferRequest = http.request(				// import { request } from 'http';	// 创建一个 'http.request' 对象 【流？】
				...
				( res ) => {
					...
					if ( showResponse ) {
						if ( debug ) {
							console.log( "\tRes - " + "Code: ".bBlue, res.statusCode );
							// console.log( res );
